    ...
    internal static class Localization
    {
        ...
        private static readonly Dictionary<string, string> English =
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Unbound", "Unbound" },
                { "InputHelp", "While free camera is active, movement keys (W/A/S/D/Q/E by default) and the auto-FOV key (= by default) are temporarily suppressed in the game's Craft and Character keyboard maps. Mouse, joystick, and right-mouse vehicle controls remain available." },
        ...
        private static readonly Dictionary<string, string> SimplifiedChinese =
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Unbound", "未绑定" },
                { "InputHelp", "自由相机启用期间，移动键（默认 W/A/S/D/Q/E）及自动 FOV 键（默认 =）会在游戏的载具和人物键盘映射中临时禁用；鼠标、手柄及右键载具操控不受影响。" },
                { "CameraLauncher", "相机" },
                { "CollapseCamera", "收起相机" },
                { "QuickTitle", "自由相机快捷控制" },
