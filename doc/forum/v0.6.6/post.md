# [Plugin Release][Windows] SP2 Free Camera v0.6.6 — Smooth Zoom with Per-Frame Target Lock
...
Version **v0.6.6** keeps the immediate moving-target lock introduced in the previous release while restoring smooth mouse-wheel zoom. Target rotation and FOV zoom now have explicitly independent smoothing paths:

- for a moving focus target, the camera reads the latest available target position and points directly at it on every rendered frame;
- moving-target focus does not smooth or translate the camera, so the view does not lag behind the target;
- mouse-wheel FOV changes still use the **FOV smoothing time** configured in the menu, including while a moving target is focused;
- FOV smoothing changes only the zoom/projection and does not delay the focus direction.
...
The plugin includes English and Simplified Chinese interfaces. Movement speed, movement smoothing, look sensitivity, FOV, FOV smoothing, terrain-focus smoothing, UI settings, and key bindings can be changed from the settings menu.

## Download

- **Ready-to-install package:** [SP2FreeCamera-v0.6.6-win-x64.zip](https://github.com/EEEureka/SP2FreeCamera/releases/download/v0.6.6/SP2FreeCamera-v0.6.6-win-x64.zip)
- **Release page:** [SP2 Free Camera v0.6.6](https://github.com/EEEureka/SP2FreeCamera/releases/tag/v0.6.6)
...
- **Checksum file:** [SHA256SUMS.txt](https://github.com/EEEureka/SP2FreeCamera/releases/download/v0.6.6/SHA256SUMS.txt)
...
2. Download `SP2FreeCamera-v0.6.6-win-x64.zip` from the link above.
